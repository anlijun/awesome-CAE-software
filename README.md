...
| [YJViewer](https://yjviewer.simversus.com/) | Web | Commercial | Web-based viewer for CAD/CAE models, Supported formats: STEP (.stp, .step)、IGES (.igs, .iges)、STL (.stl)、OBJ (.obj)、FBX (.fbx)、VTK (.vtk, .vtp)、Nastran (.bdf, .op2)、Abaqus (.inp, .odb)、Ansys (.cdb, .rst)、glTF (.gltf, .glb)、Fluent(.msh, .cas & .dat, .cas.gz & .dat.gz, .cas.h5 & .dat.h5)、Ls-Dyna（.k, d3plot）、Tecplot (.dat, .plt) |
...
| [TecIO Library](https://tecplot.com/products/tecio-library/) | C++ | Commercial | TecIO is Tecplot’s I/O library that enables reading, writing, and exchanging data in Tecplot .plt/.szplt file formats for seamless integration with simulation codes. Code: https://github.com/search?q=tecio&type=repositories |
---
